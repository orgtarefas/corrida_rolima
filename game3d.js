...
function init3D(distance) {
    if (typeof THREE === 'undefined') {
        ...
        alert('Erro ao carregar gráficos. Recarregue a página.');
        return false;
    }
    ...
    try {
        ...
        const leftEdge = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.1, 200), edgeMat);
        leftEdge.position.set(-2.4, -0.15, -100);
        scene.add(leftEdge);
        
        const rightEdge = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.1, 200), edgeMat);
        rightEdge.position.set(2.4, -0.15, -100);
        scene.add(rightEdge);
        ...
        const body = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.4, 1.4),
            new THREE.MeshStandardMaterial({ color: 0xff6b35, metalness: 0.6, roughness: 0.3 }));
        body.position.y = 0.2;
...
function startAnimation() {
    ...
    function animate() {
        ...
        if (typeof raceActive !== 'undefined' && raceActive && !gameOver && typeof raceStarted !== 'undefined' && raceStarted) {
            updateGame3D();
        }
...
function updateGame3D() {
    ...
    if (distEl) distEl.innerText = Math.floor(playerCar.distancia);
    if (speedEl) speedEl.innerText = Math.floor(playerCar.velocidade);
    if (duraEl) duraEl.innerText = Math.max(0, Math.floor(playerCar.durability));
    if (remEl) remEl.innerText = Math.max(0, Math.floor(raceDistanceTotal - playerCar.distancia));
    
    // Gerar obstáculos
    if (Math.random() < 0.018 && raceStarted && !gameOver && raceActive) {
    ...
    obstacles3d.forEach(obs => {
        if (obs.userData.active) {
            ...
            if (!invincible && carMesh &&
                ...
                if (playerCar.durability <= 0 && !gameOver) {
                    gameOver = true;
                    raceActive = false;
                    finishRace();
                }
...
function stopGame3D() {
    ...
    const controls = document.getElementById('raceControls');
    if (hud) hud.style.display = 'none';
    if (controls) controls.style.display = 'none';
    console.log('🛑 Jogo 3D parado');
}
